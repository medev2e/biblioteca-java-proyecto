...
public class BookDAO {
    ...
    public void insertBook(BookModel book) {
        ...
        String sql = """
                INSERT INTO books (system_id, title, author, publisher, publication_year, genre, isbn_number, edition, available)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """;

        try (Connection conn = DatabaseConfig.getLibraryConnection();
                PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, book.getSystemId());
            ...
            ps.setInt(9, book.getAvailable() ? 1 : 0);
            ps.executeUpdate();
        } catch (Exception e) {
        }
    }
    ...
    public void updateBook(BookModel book) {
        String sql = """
                ...
                genre = ?, isbn_number = ?, edition = ?, available = ?
                WHERE system_id = ?
                """;
        ...
        try (Connection conn = DatabaseConfig.getLibraryConnection();
            ...
            ps.setString(9, book.getSystemId());
            ps.executeUpdate();
        } catch (Exception e) {
        }
    }

    public void deleteBook(String systemId) {
        String sql = "DELETE FROM books WHERE system_id = ?";
